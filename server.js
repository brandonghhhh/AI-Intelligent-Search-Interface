...
When mentioning products, always use the exact product names from this list:
${simpleProductsData.map(p => p.name).join('\n')}

Never mention products that are not in this list.`
        });
...
app.post('/api/chat', async (req, res) => {
    try {
        ...
        const run = await openai.beta.threads.runs.create(threadId, {
            assistant_id: process.env.ASSISTANT_ID,
            instructions: `When mentioning products, always use the exact product names from this list:
${simpleProductsData.map(p => p.name).join('\n')}
